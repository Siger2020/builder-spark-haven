...
function seedDatabase() {
  try {
    ...
    if (userCount.count === 0) {
      ...
      const users = [
        ['د. كمال الموالي', 'doctor@dkalmoli.com', 'hashed_password_123', '00967777775545', 'admin', 'male', 'صنعاء، اليمن'],
        ['د. سارة أحمد', 'sara@dkalmoli.com', 'hashed_password_123', '00967771234567', 'doctor', 'female', 'صنعاء، اليمن'],
        ['أحمد محمد علي', 'patient1@test.com', 'hashed_password_123', '00967772345678', 'patient', 'male', 'صنعاء، حي الحصبة'],
        ['فاطمة خالد', 'patient2@test.com', 'hashed_password_123', '00967773456789', 'patient', 'female', 'صنعاء، حي الثورة'],
        ['محمد سا��م', 'patient3@test.com', 'hashed_password_123', '00967774567890', 'patient', 'male', 'صنعاء، حي السبعين'],
        ['نورا أحمد', 'receptionist@dkalmoli.com', 'hashed_password_123', '00967775678901', 'receptionist', 'female', 'صنعاء، اليمن']
      ];
      ...
      insertDoctor.run(1, 'DOC001', 'طبيب أسنان عام وتجميل', 'LIC001', 'بكالوريوس طب الأسنان - جامعة صنعاء', 15, 50);
      insertDoctor.run(2, 'DOC002', 'تقويم الأسنان', 'LIC002', 'ماجستير تقويم الأسنان', 8, 75);
      ...
      insertPatient.run(3, 'PAT001', 'شركة التأمين الوطنية', 'لا توجد مشاكل صحية كبيرة', 'لا توجد حساسيات معروفة', 'O+', 1);
      insertPatient.run(4, 'PAT002', 'التأمين الحكومي', 'ارتفاع ضغط الدم', 'حساسية من البنسلين', 'A+', 2);
      insertPatient.run(5, 'PAT003', null, 'داء السكري من النوع الثاني', 'حساسية من الأسبرين', 'B+', 1);
      ...
      insertAppointment.run('APP001', 1, 1, 1, today, '09:00', 'confirmed', 'تنظيف دوري');
      insertAppointment.run('APP002', 2, 2, 3, tomorrow, '10:30', 'scheduled', 'استشارة تقويم');
      insertAppointment.run('APP003', 3, 1, 2, tomorrow, '14:00', 'scheduled', 'ألم في الضرس');
      ...
      insertTransaction.run('TXN001', 1, 'payment', 50, 'cash', 'completed', 'تنظيف الأسنان');
      insertTransaction.run('TXN002', 2, 'charge', 200, 'card', 'pending', 'استشارة تقويم');
      insertTransaction.run('TXN003', 3, 'payment', 100, 'cash', 'completed', 'حشوة سن');
...
export function getDatabaseStats() {
  const tables = [
    'users', 'patients', 'doctors', 'appointments', 'services',
    'medical_reports', 'treatment_plans', 'treatment_sessions',
    'financial_transactions', 'invoices', 'notifications', 'inventory'
  ];
...
export async function createBackup(backupName?: string) {
  try {
    ...
    insertBackup.run(name, 'full', backupPath, 'completed', new Date().toISOString());
...
// تسجيل النشاط
export function logActivity(userId: number, action: string, entityType?: string, entityId?: number, oldValues?: any, newValues?: any) {
  try {
...
export function globalSearch(query: string, limit = 50) {
  ...
  try {
    ...
    const patients = patientSearch.all(searchTerm, searchTerm, searchTerm, searchTerm, Math.floor(limit / 4));
    ...
    const appointments = appointmentSearch.all(searchTerm, searchTerm, Math.floor(limit / 4));
    ...
    const transactions = transactionSearch.all(searchTerm, searchTerm, Math.floor(limit / 4));
