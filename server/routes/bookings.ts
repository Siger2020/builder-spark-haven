...
router.post('/', async (req, res) => {
  try {
    ...
    try {
      ...
      try {
        ...
        if (email) {
          ...
          if (existingEmailUser) {
            finalEmail = `${email.split('@')[0]}_${Date.now()}@${email.split('@')[1]}`;
            console.log(`📧 البريد الإلكتروني موجود (المحاو��ة الثانية), سيتم استخدام: ${finalEmail}`);
          }
        }
        ...
        if (secondError.code === 'SQLITE_CONSTRAINT_UNIQUE') {
          errorMessage = '��بدو أن هناك حساب موجود بنفس البيانات. يرجى المحاولة مرة أخرى.';
        }
    ...
    console.log(`📧 بدء إرسال إشعارات الحجز للمريض: ${name} - البريد: ${email || 'غير محدد'}`);
